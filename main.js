...
class Game {
    ...
    handleDrop(e, x, y, boardType) {
        ...
        const dragged = this.draggedResource.resource;
        const board = boardType === 'main' ? this.board : this.stageBoard;
        const targetResource = board[y][x];
        const origX = this.draggedResource.originalX;
        const origY = this.draggedResource.originalY;

        // 1. 드롭 위치에 자원이 있으면 교환
        if (targetResource && targetResource !== dragged) {
            this.swapResource(dragged, targetResource, boardType);
        }
        // 2. 빈 칸이면 이동
        else if (this.canPlaceResource(x, y, boardType)) {
            this.moveResource(dragged, x, y, boardType);
        }
        // 3. 그 외(장애물 등)면 원위치 복귀
        else {
            // 원래 자리로 이동
            this.moveResource(dragged, origX, origY, boardType);
        }
     ...
     * 두 자원 위치 교환
     */
    swapResource(resourceA, resourceB, boardType) {
        if (!resourceA || !resourceB) return;
        const board = boardType === 'main' ? this.board : this.stageBoard;
        // 위치 정보 스왑
        const tempX = resourceA.x, tempY = resourceA.y;
        resourceA.x = resourceB.x;
        resourceA.y = resourceB.y;
        resourceB.x = tempX;
        resourceB.y = tempY;
        // 보드 배열 스왑
        board[resourceA.y][resourceA.x] = resourceA;
    ...
    /**
     * 해당 위치에 자원이 있는지(충돌) 반환
     */
    isCollision(x, y, boardType) {
        const board = boardType === 'main' ? this.board : this.stageBoard;
        return !!(board[y] && board[y][x]);
    }
}
